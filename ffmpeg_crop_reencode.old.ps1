...
# If 'filters' is empty or undefined, use default filters [1, 2, 3]
if (-not $filters) {
    $filters = 'crop=1080:1080:700:0,eq=brightness=0.1,unsharp=7:7:1.5:7:7:0.0'
}
...
# Only process files in the main folder, not subfolders
$files = Get-ChildItem -Path $inputDir -Filter *.mp4 -File -Depth 0
if ($files.Count -eq 0) {
...
foreach ($file in $files) {
    ...
    Write-Progress -Activity "Processing MP4 files" -Status ("{0} of {1} ({2}%)" -f $counter, $total, $percent) -PercentComplete $percent
    ...
    # Run ffmpeg directly to see progress
    & ffmpeg @ffmpegArgs
    if ($LASTEXITCODE -ne 0) {
        Write-Host "Error processing $($file.Name)" -ForegroundColor Red
    }
}
Write-Progress -Activity "Processing MP4 files" -Completed
